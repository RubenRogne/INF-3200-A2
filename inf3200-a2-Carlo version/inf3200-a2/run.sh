...
# ---- killall mode (stop all old servers and wait until they are gone)
if [[ "${1-}" == "--killall" ]]; then
  ...
  for N in $(cat /share/compute-nodes.txt); do
    ...
    ssh -o ConnectTimeout=2 -o ConnectionAttempts=1 -x "$N" "
      # try gentle kill first (TERM up to 5 times)
      ...
      # if still running after gentle tries, do a hard kill
...
# ---- kill servers if requested (cleanup after run)
