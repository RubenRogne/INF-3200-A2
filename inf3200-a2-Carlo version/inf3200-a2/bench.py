...
def now_s():
    ...
    # perf_counter gives high-res time
    t = time.perf_counter()
    return t

def rand_text(n):
    # build a random string of length n
    # control the payload size for PUT
...
def main():
    ...
    # --- replace your argparse block with this 
    ...
    if args.peers != "":
        try:
            ...
            text = f.read()
            f.close()
            data = json.loads(text)           # expects ["HOST:PORT","HOST:PORT"..] from a json file
            i = 0
            while i < len(data):
    ...
    print("[info] nodes: " + str(nodes)) #safety check prints
    ...
    while run_idx < args.repeats: #counts which run you are on
