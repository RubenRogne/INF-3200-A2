...
# use pow doubling to simulate log2(n) (how many times we divide n by 2 until we reach 1)
# (count how many times we double 1 until reaching node_count)
# then add +1 finger for safety (rounding + extra shortcut)
# ex: n=4 = 3 fingers, n=8 = 4 fingers, n=10 = 5 fingers
...
class ChordNode:
    def __init__(self, self_address, peer_addresses):
        ...
        print(f"[DEBUG] {self.self_address} pred={self.pred_address} succ={self.succ_address}") #just debug helper
        ...
        for i in range(finger_count): #do for numbers of fingers needed
            ...
            for j in range(len(ring)): #loop in the sorted ring itself
                ...
                node_id = ring[j][1] # [1] or [0] is which part of the couple is [(addr,ID)] so addr or ID  
        ...
        # so this node only keeps pred, succ, and fingers to self (each node only knows what is needed)
    ...
    def shortcut_step(self, target_id):
        ...
        while index >= 0:
            ...
            if node_address != self.self_address: #check that is not himself 
                # (could cause infinite loop if himself in table, could happen in small rings)
